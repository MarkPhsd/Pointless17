...
export class ReportingItemsSalesService {
  ...
  getItemsToPrep(site: ISite, search: IReportingSearchModel): Observable<TaxSalesReportResults> {

    // let search = {locationID: locationID} as IReportingSearchModel
    // search.itemPrepped  = 1;
    search.pageNumber   = 1;
    search.pageSize     = 50;
   
    const controller = `/ReportItemSales/`

    const endPoint = `getItemsToPrep`

    const parameters = ``

    const url = `${site.url}${controller}${endPoint}${parameters}`

    return  this.http.post<TaxSalesReportResults>(url, search )
