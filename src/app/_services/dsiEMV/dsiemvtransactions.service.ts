...
export class DSIEMVTransactionsService {
  ...
  async mercuryPinPadReset(transaction: Transaction): Promise<CommandResponse> {
    ...
    const dsiResponse = {} as CommandResponse;
    return dsiResponse;
  }

  async pinPadReset(transaction: Transaction): Promise<CommandResponse> {
    if (!transaction) { return }
    const tstream       = {} as TStream;
    tstream.Transaction = transaction
  ...
  async emvTransaction(transaction: Transaction): Promise<CommandResponse> {
    if (!transaction) { return }
    const tstream       = {} as TStream;
    tstream.Transaction = transaction
    const topLevel      = {} as topLevel;
