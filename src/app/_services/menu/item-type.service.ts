...
export class ItemTypeService {
  ...
  private _itemTypeList     = new BehaviorSubject<IItemType[]>(null);
  public itemTypeList$      = this._itemTypeList.asObservable();

  itemTypeList = [] as IItemType[];
  // _list
  prepList =  [{id: 0, name: ''}, {id: 1, name: 'Prep 1'}, {id: 2, name: 'Prep 2'}];
...
// 13	Restaurant
  ...
  getItemTypeFromList(id: number): IItemType { 
    ...
    if (!list) { return null}
    return list.filter(data => { return data.id == id })[0]
    // return item[0];
  }

  public getTypeList(site: ISite)   : Observable<IItemType[]> {
  ...
  initItemTypes(site: ISite, iItemType: IItemType[]): Observable<IItemType[]> {
    ...
    return  this.http.post<any>(url, iItemType).pipe(switchMap(data => {
      if (data) {
        if (data.errorMessage) {
          this.siteService.notify(data?.errorMessage, 'Error', 5000, 'red')
        }
      }
  ...
  getDefaultItemTypes(): IItemType[]  {
    ...
    productType =
      ...
      {id: 2,   portionValue       : '', autoAddJSONProductList: '', useDefaultPriceInApp: true,
            ...
            printerLocations   : printerLocations, enableCustomNote: false, wicebt: 0, description: '', requireProductCount: false,
      ...
      {id: 3,   portionValue       : '', autoAddJSONProductList: '', useDefaultPriceInApp: true,
            packagingMaterial: '', promptQuantity: false, description: '', webStoreSimpleView: false, requireProductCount: false,
            useGroups          : useGroup, unitName: '', pricePrompt: false, disablePriceCategory: false, disableSimplePrice: false,
            printerLocations   : printerLocations, enableCustomNote: false, wicebt: 0,
